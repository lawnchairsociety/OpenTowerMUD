...
func (d *Database) RecordBossKill(towerID, playerName string) (bool, error) {
	...
	var count int
	err = tx.QueryRow(d.qb.Build(`SELECT COUNT(*) FROM boss_kills WHERE tower_id = ?`), towerID).Scan(&count)
	if err != nil {
		return false, err
	}
...
func (d *Database) HasTowerBeenDefeated(towerID string) (bool, error) {
	var count int
	err := d.db.QueryRow(d.qb.Build(`SELECT COUNT(*) FROM boss_kills WHERE tower_id = ?`), towerID).Scan(&count)
	if err != nil {
		return false, err
	}
	return count > 0, nil
